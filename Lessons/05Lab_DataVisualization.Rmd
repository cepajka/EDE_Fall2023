...
---
##changing the color outside aes makes the colors change. colors inside the aesthetics, you are color coding your data. so you assign a column to color, and R will populate that column with different colors. normally its a factor
## Objectives
...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    ) #need to specify 3 things with geom text. you need a value for x and y, and that will be the position of your text. you also need the label, which is the text you want to add to your plot. if not using expressions, just put your text straight into label=. if you have expression, you can use like stat summaries.

#Print the plot
print(Plot_text)
#have to ask r to print the plot. you can assign it, but it won't print it 
#Save the plot
ggsave(
