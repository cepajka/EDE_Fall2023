...
vector1[3] # square brackets will tell you what the 3rd element of vector 1 will be. vectors are 1 dimensional. only 1 number representing th index.

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # matrices have 2 dimensions. provide the matrix the sequence of numbers like 1-20, and the number of rows and columns you want
matrix1
...
matrix1[4, ] # in respect to the row is before comma
matrix1[ , 3] # after comma is in respect to the column
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] # specifying rows 2-4 and columns 1 through 3. 
...
```{r DataExercise-1a} 
hightempsF <- c(82, 83, 84, 79, 71, 78, 82, 85, 84, 82, 80, 79, 80, 80)
lowtempsF <- c(56, 58, 60, 60, 62, 62, 64, 64, 62, 60, 58, 59, 58, 58)
```
...
```{r DataExercise-1b}
high_C <- (hightempsF -32)*5/9
low_C <- (hightempsF - 32)*5/9
}
```
...
```{r DataExercise-1c}
df_temp <- data.frame("HighTempF"=hightempsF, high_C, lowtempsF, low_C) # the renaming with ""=
```
...
```{r DataExercise-1d}
summary(df_temp)
sd(df_temp$HighTempF) #this one works but you will get an error if no $ because you need to give sd a single vector aka a column of the data set, so sd(df_temp) won't wok
```
