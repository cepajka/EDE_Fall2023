...
class(NTL.phys.data$depth) #grabbing all the rows where the depth column
...
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304))#could also use the unique function or the level function
```
...
filter(NTL.phys.data, year4 %in% c(1999)) #can also do year4 ==1999
summary(filter(NTL.phys.data, year4 %in% c(1999)))
# Exercise 2: 
...
summary(filter(NTL.phys.data, lakename %in% ("Tuesday Lake"), year4 %in% c(1990:1999)))
