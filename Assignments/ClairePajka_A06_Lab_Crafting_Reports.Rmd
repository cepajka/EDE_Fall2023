...
Date          | 2018-2019
Filename      | EPAair_O3_PM25_NC1819.csv
...
```{r project.setup, message=FALSE, warning=FALSE, include=TRUE, results=FALSE}
#Import libraries
...
epa_data <- read.csv(
 here("C:/Users/cepaj/OneDrive/Documents/EDE_Fall2023/Data/Processed_KEY/Processed_KEY/EPAair_O3_PM25_NC1819_Processed.csv"),
 stringsAsFactors = TRUE) %>%
 mutate(Date = ymd(Date))
...
knitr::kable(summary_PM25,caption = "Mean Ozone", col.names= c("County", "$\\mu g/m^3$"))
...
```{r Ozone_plot_2018, echo=FALSE, warning=FALSE, fig.height=3.5, fig.align= "left", fig.cap= "Ozone Concentration by Month in 2018"}
my_theme <- theme_classic(base_size = 12)
theme_set(my_theme)
Monthly_Ozone_plot_2018 <-
  ggplot(filter(epa_data, Year ==2018), 
         aes(x=factor(Month, levels = 1:12, labels = month.abb),
             y = Ozone))+
  geom_boxplot()+
  xlab("Month")+
  ylab(expression(paste("Concentration(", mu, "g/m"^"3", ")")))+
  ggtitle("2018 Ozone Distribution by Month")+
  scale_x_discrete(name="Month", drop=FALSE)+
  theme(axis.text.x = element_text(angle = 45, hjust=1))+
  theme(plot.title = element_text(hjust = 0.5))
print(Monthly_Ozone_plot_2018)

```

```{r Ozone_plot_2019, echo=FALSE, warning=FALSE, fig.height=3.5, fig.align="left", fig.cap="Ozone Concentration by Month in 2019"}
Monthly_Ozone_plot_2019 <-
  ggplot(filter(epa_data, Year ==2019), 
         aes(x=factor(Month, levels = 1:12, labels = month.abb),
             y = Ozone))+
  geom_boxplot()+
  xlab("Month")+
  ylab(expression(paste("Concentration(", mu, "g/m"^"3", ")")))+
  ggtitle("2019 Ozone Distribution by Month")+
  #labs(title="2019 Ozone Distribution by Month",
   #    fontface = "bold",
    #   x= "Month",
...
## Git Repository
https://github.com/cepajka/EDE_Fall2023
